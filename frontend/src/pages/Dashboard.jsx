...
function PriorityBadge({ score }) {
  const level = score >= 70 ? ['critical', 'var(--red)', '#fee2e2'] :
                score >= 40 ? ['high', '#c2410c', '#ffedd5'] :
                score >= 15 ? ['medium', 'var(--amber)', 'var(--amber-bg)'] :
                              ['low', 'var(--green)', 'var(--green-bg)']
  return (
    <span style={{
      background: level[2], color: level[1],
  ...
  return (
    <div>
        ...
        <div className="card mb-16" style={{ marginBottom: 16, borderColor: 'var(--red)', borderWidth: 2 }}>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <div className="checkbox-group">
                ...
                {zones.length === 0 && <span className="text-sm text-muted">No zones found</span>}
              </div>
            </div>
          </div>
          <div className="form-group">
            <label className="form-label">Description (optional)</label>
            <input className="form-input" placeholder="Brief description of the situation…" value={crisisForm.description} onChange={e => setCrisisForm(f => ({ ...f, description: e.target.value }))} />
          </div>
          <div className="alert alert-warning">
            This will generate a check-in task for every household in the selected zones. Make sure this is a real emergency.
          </div>
          <div className="row" style={{ gap: 8, marginTop: 8 }}>
            <button className="btn btn-danger" onClick={activateCrisis} disabled={activatingCrisis || crisisForm.affected_zones.length === 0}>
              {activatingCrisis ? 'Activating…' : '🚨 Confirm Activation'}
            </button>
      ...
      <div className="grid-4" style={{ marginBottom: 16 }}>
        ...
        <div className="stat-card">
          <div className="stat-value" style={{ color: 'var(--red)' }}>{highPriority.length}</div>
          <div className="stat-label">High Priority</div>
        </div>
        <div className="stat-card">
          <div className="stat-value" style={{ color: 'var(--green)' }}>{helpers.length}</div>
          <div className="stat-label">Potential Helpers</div>
        </div>
        <div className="stat-card">
          <div className="stat-value">{activeCrises.length}</div>
      ...
      <div className="card">
        <div className="card-header">
          <div>
            <div className="card-title">Zone Households</div>
            <div className="card-subtitle">Sorted by vulnerability priority — highest risk first</div>
          </div>
        </div>
        <div className="stack">
          {households.length === 0 && (
            <div className="empty-state">
              ...
              <div>No households registered yet.</div>
            </div>
          )}
          {households.map(h => (
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="row flex-wrap gap-8 mt-4">
                      ...
                      {h.medical_equipment.length > 0 && (
                        <span className="text-xs" style={{ background: 'var(--amber-bg)', padding: '1px 6px', borderRadius: 4, color: 'var(--amber)' }}>
                          ⚡ Medical equip.
                        </span>
                      )}
                      {h.languages[0] !== 'English' && (
